...
public class SwapWorkerTest {
    ...
    @Test
    public void move_swapWorker() throws SantoriniException {
        Cell cellToMove1_1 = initCellWorker2_2;
        strategyMove.move(worker1_1, cellToMove1_1);
        assertEquals(worker1_1, match.getLocation().getOccupant(cellToMove1_1));
        assertEquals(worker2_2, match.getLocation().getOccupant(initCellWorker1_1));
    }

    @Test (expected = WrongCellSelectedMoveException.class)
    public void move_swapAlliedWorker_throwsWrongCellSelectedMoveException () throws SantoriniException {
